...
docker build -t rex-proxybuilder:1.7.1 -f buildtools/Dockerfile.proxybuilder .
...
docker run -it --mount type=bind,source=${REX_ISTIO_ENVOY_PATH},target=/envoy --mount type=bind,source=${PWD},target=/proxy --name proxybuilder rex-proxybuilder:1.7.1 /bin/bash
